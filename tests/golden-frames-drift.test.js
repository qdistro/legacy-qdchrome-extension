...
// This repo is qdchrome-extension; its sibling is qdfirefox-extension.
  ...
  it("matches the sibling extension's golden fixture byte-for-byte", () => {
    if (!fs.existsSync(SIBLING)) {
      ...
      expect(
        ...
        "check out qdfirefox-extension side-by-side or set $QDISTRO_SIBLING_GOLDEN.",
      ).toBe(false);
      // eslint-disable-next-line no-console
      console.warn(
        `[golden-frames.drift] sibling fixture not found at ${SIBLING}; ` +
        "cross-repo byte-identity NOT verified in this run. Set " +
        "$QDISTRO_SIBLING_GOLDEN or check out qdfirefox-extension " +

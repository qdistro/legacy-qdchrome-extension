...
// Install a listener-wrapping addEventListener for the whole test so
// BOTH the content script's document-level handlers AND the picker
// row handlers (registered later, in renderPicker) observe the
// per-dispatch trust flag.
...
// wrapped by load() observe event.isTrusted === `trusted`.
...
describe("pwd-content.js", () => {
  ...
  it("a trusted click on a non-password input does NOT fire request_fill", async () => {
    ...
    // Put the text input outside any form so it can't resolve a pw field.
  ...
  it("single-credential reply renders a confirmation picker and does NOT auto-fill (finding #10)", async () => {
    ...
    userClick(p);
    await tick();
    // Nothing written to the DOM yet — a picker is shown for confirmation.
    expect(p.value).toBe("");
    expect(u.value).toBe("");
  ...
  it("an UNTRUSTED picker-row mousedown does NOT fill (finding #10)", async () => {
    ...
    const picker = document.getElementById("qdistro-pwd-picker");
    expect(picker).toBeTruthy();
    // Page script tries to harvest by faking the row click.
  ...
  it("form submit after a confirmed fill does NOT fire pwd.request_save when value is unchanged", async () => {
    ...
    userClick(p);
    await tick();
    // Confirm the fill via a trusted picker-row click.
    const picker = document.getElementById("qdistro-pwd-picker");
    dispatchAs(picker.children[0],
      new MouseEvent("mousedown", { bubbles: true, cancelable: true }), true);
    await tick();
  ...
  it("phase 1 (pwd.request_fill) NEVER receives a password; the fill comes from phase 2", async () => {
    // Raw phase-1 reply exactly as the daemon shapes it: metadata +
    // fill_token, NO password on the rows. Drive the mock directly so
    // there is no chance a password leaks via the row.
    env.chrome.runtime.sendMessage = (msg) => {
      env.sent.push(msg);
      if (msg.kind === "pwd.request_fill") {
        return Promise.resolve({
          ok: true,
      ...
      if (msg.kind === "pwd.request_fill_confirm") {
        // Assert the confirm round-trip carries exactly what the daemon
        // binds the token to.
    ...
    const picker = document.getElementById("qdistro-pwd-picker");
    expect(picker).toBeTruthy();
    // Nothing filled yet — phase 1 carried no password.
    expect(p.value).toBe("");
    dispatchAs(picker.children[0],
      new MouseEvent("mousedown", { bubbles: true, cancelable: true }), true);
    await tick();
    // The phase-2 confirm delivered the real secret.
    expect(u.value).toBe("alice");
  ...
  it("a denied/expired fill_confirm does NOT fill the field", async () => {
    ...
    // The daemon refuses to release the secret (e.g. token expired).
    ...
    // No secret was released, so nothing landed in the DOM.
    expect(p.value).toBe("");
    expect(u.value).toBe("");
  });
  ...
  it("a fill_confirm whose username disagrees with the pick is REJECTED (no fill)", async () => {
    ...
    // A buggy/compromised daemon path returns a different account's
    // secret — refuse rather than fill the wrong credential.
    env.setConfirmReply({
      ok: true,
